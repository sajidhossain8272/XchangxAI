...
export default function Hero() {
  ...
  const [rates, setRates] = useState<Record<Fiat, number> | null>(null); // 1 USD -> CCY
    ...
    const fetchRates = async () => {
      try {
        ...
        const { data } = await axios.get(
          ...
          { params: { base: "USD", symbols: "USD,EUR,BDT" } }
        );

        const rUSD = 1;
        const rEUR = data?.rates?.EUR ?? null;
        const rBDT = data?.rates?.BDT ?? null;
        if (!rEUR || !rBDT) throw new Error("Missing EUR/BDT rates");

        const next: Record<Fiat, number> = { USD: rUSD, EUR: rEUR, BDT: rBDT };
        if (mounted) {
          setRates(next);
  ...
  const effectiveSrcToDstRate = useMemo(() => {
    ...
    const srcToUsd = 1 / (rates[srcCcy] || 1);
